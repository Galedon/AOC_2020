...
pub fn get_day_9() -> Result<String, Box<dyn std::error::Error>>{
...
pub fn get_day_11() -> Result<String, Box<dyn std::error::Error>>{
    ...
    fn get_occ_final(method: fn(x: i32, y: i32, array_old: &Vec<Vec<char>>) -> i32) -> Result<i32, Box<dyn std::error::Error>>
    {
        ...
        fn one_seat(x: usize, y: usize, array: &mut Vec<Vec<char>>, array_old: &Vec<Vec<char>>,
                    method: fn(x: i32, y: i32, array_old: &Vec<Vec<char>>) -> i32) -> i32 {
            let mut toll = 4;
            if method == get_occupied_visible {toll = 5}
            let mut n_changed = 0;
            if (array_old[x][y] == 'L') && (method(x as i32, y as i32, array_old) == 0)
            {
                array[x][y] = '#';
                n_changed += 1;
            } else if (array_old[x][y] == '#')
                && (method(x as i32, y as i32, array_old) >= toll)
            {
                array[x][y] = 'L';
                n_changed += 1
            }
        ...
        fn one_iteration(
            ...
            method: fn(x: i32, y: i32, array_old: &Vec<Vec<char>>) -> i32
        ) -> i32 {
            let mut n_changed = 0;
            for i in 0..array_old.len() {
                for j in 0..array_old[i].len() {
                    n_changed += one_seat(i, j, array, array_old, method);
                }
            }
            n_changed
        }
        ...
        loop {
            let n_changes = one_iteration(&mut array, &mut array_old, method);
            if n_changes == 0 {
                break;
            }
    ...
    let res_a = get_occ_final(get_occupied_neighbours);
    let res_b = get_occ_final(get_occupied_visible);
    Ok(format!("res_a = {}, res_b =  {}", res_a.unwrap(), res_b.unwrap()))
